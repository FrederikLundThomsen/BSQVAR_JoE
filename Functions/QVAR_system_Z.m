...
function [b_T,D_p,A0_p,AL_p,BL_p,V_T,eps] = QVAR_system_Z(Y,P,X,S,D,Quant,ZeroRestrictions)
...
for i=1:K
    ...
    for j=1:n
        %Estimation
        [b,~,u,~]= QVAR_Estim_Z(Y,P,X,S,D,Quant(j),i,ZeroRestrictions);
        ...
        if i > 1
            A0_p(i,1:i-1,j) = b(end-(i-2):end);
        end
        
        if P > 0
            %Endogenous lags
            AL_p(i,:,j) = b(1+C:(C+K*P));
        end
    
        %Exogenous variables
        if isempty(X) == 0
            BL_p(i,:,j) = b(1+C+K*P:C+K*P+M*(S+1));
        end
